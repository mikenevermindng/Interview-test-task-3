...
func GetServiceAccessTime(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"success": true, "service": heartbeat.Uri, "responseTime": heartbeat.ResponseTime, "unit": "ms", "status": heartbeat.Status})
	return
}
...
func GetServiceWithMaxAccessTime(c *gin.Context) {
	...
	err := dbClient.
		...
		Limit(1).
		Select("h.uri as uri, h.response_time as response_time").
		Scan(&serviceResponseTime).Error
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "Something went wrong"})
		return
	}
	c.JSON(http.StatusOK, gin.H{"success": true, "service": serviceResponseTime.Uri, "responseTime": serviceResponseTime.ResponseTime, "unit": "ms", "status": serviceResponseTime.Status})
	return
}
